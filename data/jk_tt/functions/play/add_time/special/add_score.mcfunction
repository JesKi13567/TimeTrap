# 分数加到对应目标上
advancement grant @s only jk_tt:clock/learn
# 按难度倍数 by difficuty
# *2
execute if score #init_diff jk_tt_mem matches 1 run scoreboard players operation @s jk_tt_timeadd *= #2 jk_tt_mem
# *3/2=1.5
execute if score #init_diff jk_tt_mem matches 2 run scoreboard players operation @s jk_tt_timeadd *= #3 jk_tt_mem
execute if score #init_diff jk_tt_mem matches 2 run scoreboard players operation @s jk_tt_timeadd /= #2 jk_tt_mem
