...
execute if score #lang jk_tt_mem matches 1 run tellraw @a ["",{"text":">> ","color":"gold"},{"selector":"@p[tag=jk_tt_murder]"}," killed ",{"selector":"@s"},", so time + ",{"score":{"name":"#temp","objective":"jk_tt_timeadd"},"color":"yellow"}]